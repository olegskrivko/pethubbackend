...
class PushSubscriptionViewSet(viewsets.ModelViewSet):
    ...
    Handles subscription, unsubscription, and notification sending.
    """
    ...
    def subscribe(self, request):
        """
        Handle subscription logic (saving or updating the subscription).
        """
    ...
    def unsubscribe(self, request):
        """
        Handle unsubscription logic (removing the subscription).
        """
        if not request.user.is_authenticated:
            return Response(
                {"detail": "Authentication required."}, 
    ...
    def is_subscribed(self, request):
        """
        Check if the current user is subscribed (filtered by endpoint).
        """
        endpoint = request.query_params.get("endpoint")
        if not endpoint:
            return Response(
                {"error": "Missing 'endpoint' query param."}, 
                status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def user_location(self, request):
        ...
        # Get the latest PushSubscription for the user
        subscription = (
